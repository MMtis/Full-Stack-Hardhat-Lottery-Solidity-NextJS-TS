    ...
    : describe("Raffle Unit test", function () {
          ...
          describe("checkUpkeep", function () {
              it("returns false if people haven't sent any ETH", async function () {
                  ...
                  // checkUpKeep is a public function not view/pure which means that
                  // it will send a transaction on call. We need to simulate sending the transaction
                  // without actually sending it to check the value returned
                  // We can do that by using the callStatic
              ...
              it("returns false if raffle isn't open", async function () {
                  ...
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
                  await raffle.performUpkeep("0x") // "0x" hh know that this is a blank bytes object
                  const raffleState = await raffle.getRaffleState()
                  const { upkeepNeeded } = await raffle.callStatic.checkUpkeep([])
          ...
          describe("performUpkeep", function () {
              it("it can only run if checkUpkeep is true", async function () {
                  ...
                  assert(tx) // if performUpkeep fails assert will fail
              })
          ...
          describe("fulfillRandomWords", function () {
              ...
              it("picks a winner, resets the lottery, and sends the money", async function () {
                  ...
                  // Setting up and activating the listener for the event
                  await new Promise<void>(async (resolve, reject) => {
                      ...
                      // which then calls the overriden fulfillRandomWords of raffle contract
